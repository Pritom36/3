document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayComics(comics) {
        ...
        if (!comics || comics.length === 0) {
             if (searchInput.value.trim()) { // Only show no results if searching
                noResultsMessage.textContent = "No comics found matching your search.";
             } else {
                noResultsMessage.textContent = "No comics available."; // General message if data is empty
             }
        ...
        checkScrollAnimation(); // Use the function defined later
    }
    ...
    function filterComics() {
        ...
        displayComics(filteredComics); // Update displayed comics
        noResultsMessage.style.display = (filteredComics.length === 0 && searchTerm) ? 'block' : 'none';
    }
    ...
    function setupEventListeners() {
        ...
        searchInput.addEventListener('input', () => { clearTimeout(searchDebounceTimeout); searchDebounceTimeout = setTimeout(filterComics, 300); });
